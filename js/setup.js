...
var app = {
  apikey: "34550da288a14a3456929f079b816e05020440f6",
  map: L.map('map', { center: [ 40.76598152771282,-73.98056030273438,], zoom: 11 }),
  geojsonClient: new cartodb.SQL({ user: 'wagnziqin', format: 'geoJson' }),
...
//flip the lat and lon from carto sql so it will work
var sql = "SELECT * FROM world_borders as wb where ST_Contains(ST_SetSRID(wb.the_geom, 4326),ST_SetSRID(ST_Point(-99.49218749999999,36.31512514748051), 4326)  )";
// The initial query by which we map the geojson representation of a table
app.geojsonClient.execute("SELECT *FROM park_point_1 as pp where	pp.gardenid < 50") // 'LIMIT' should be added to the end of this line
  .done(function(data) {
    console.log(data);
    L.geoJson(data, {
...
var fillForm = function(properties) {
  ...
  $('#name').val(properties.gardenname);
};
